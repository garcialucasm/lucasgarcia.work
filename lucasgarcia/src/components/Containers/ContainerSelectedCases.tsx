...
function ContainerSelectedCases({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`group flex w-full flex-col items-center gap-8 rounded-2xl py-8 p-8 transition-all lg:flex-row lg:items-start ${selected === ToggleButtonOptions.BACKEND ? "bg-neutral-800" : "bg-white"}`}>
      {children}
    </div>
  )
}
